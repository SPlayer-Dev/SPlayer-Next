...
export const requestPlatformLyric = async (
  ...
  const resp = byId
    ...
    : await window.api.lyrics.matchByQuery(platform, track, options);
  return resp.ok && resp.data ? resp.data : null;
};

/**
 * 歌词一致性校验通过后写入模糊匹配缓存
 * @param confirmation - 匹配确认信息
 */
export const confirmLyricMatch = (confirmation: LyricMatchConfirmation) =>
  window.api.lyrics.confirmMatch(confirmation);
